...
post_provision_config_nodes() {
    ...
    if ! yum -y install CUnit python36-PyYAML                         \
                        python36-nose python36-pip valgrind           \
                        python36-paramiko python2-avocado             \
                        python2-avocado-plugins-output-html           \
                        python2-avocado-plugins-varianter-yaml-to-mux \
                        libcmocka python-pathlib python2-numpy git    \
                        python2-clustershell golang-bin ipmctl ndctl  \
                        patch python2-distro $INST_RPMS; then
        rc=${PIPESTATUS[0]}
