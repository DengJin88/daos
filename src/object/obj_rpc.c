				     ...
				     struct dtx_redundancy_group *drg)
{
	...
	for (i = 0, size = sizeof(*drg); i < drg->drg_tgt_cnt;
	     size += sizeof(drg->drg_index[i++])) {
		rc = crt_proc_uint32_t(proc, &drg->drg_index[i]);
		if (rc != 0)
			return -DER_HG;
	}
				 ...
				 struct daos_cpd_sub_req *dcsr)
{
	...
	case DCSO_READ: {
		...
		if (proc_op == CRT_PROC_DECODE) {
			D_ALLOC_ARRAY(dcr->dcr_akeys, dcsr->dcsr_nr);
			if (dcr->dcr_akeys == NULL)
				D_GOTO(out, rc = -DER_NOMEM);
		}

		for (i = 0; i < dcsr->dcsr_nr; i++) {
			rc = crt_proc_daos_key_t(proc, &dcr->dcr_akeys[i]);
			if (rc != 0)
				D_GOTO(out, rc = -DER_HG);
		}
...
out:
	...
	case DCSO_PUNCH_AKEY:
		...
		break;
	case DCSO_READ:
		D_FREE(dcsr->dcsr_read.dcr_akeys);
		break;
	}
...
static int
crt_proc_struct_daos_cpd_disp_req(crt_proc_t proc,
				  struct daos_cpd_disp_req *dcdr)
{
	int	rc;

	rc = crt_proc_uint32_t(proc, &dcdr->dcdr_shard_idx);
	if (rc != 0)
		return -DER_HG;

	rc = crt_proc_uint32_t(proc, &dcdr->dcdr_req_idx);
	if (rc != 0)
		return -DER_HG;
...
static int
crt_proc_struct_daos_cpd_disp_desc(crt_proc_t proc,
				   struct daos_cpd_disp_desc *dcdd)
{
	crt_proc_op_t	proc_op;
	uint32_t	count;
	int		rc;
	...
	rc = crt_proc_uint32_t(proc, &dcdd->dcdd_read_cnt);
	if (rc != 0)
		return -DER_HG;

	rc = crt_proc_uint16_t(proc, &dcdd->dcdd_write_cnt);
	if (rc != 0)
		return -DER_HG;

	rc = crt_proc_uint16_t(proc, &dcdd->dcdd_head_idx);
	if (rc != 0)
		return -DER_HG;
	...
	if (proc_op == CRT_PROC_FREE) {
		D_FREE(dcdd->dcdd_reqs);
		return 0;
	}

	count = dcdd->dcdd_read_cnt + dcdd->dcdd_read_cnt;
	if (proc_op == CRT_PROC_DECODE) {
		D_ALLOC_ARRAY(dcdd->dcdd_reqs, count);
		if (dcdd->dcdd_reqs == NULL)
			return -DER_NOMEM;
	}

	for (i = 0; i < count; i++) {
		rc = crt_proc_struct_daos_cpd_disp_req(proc,
						       &dcdd->dcdd_reqs[i]);
		if (rc != 0)
			return -DER_HG;
	}
				  ...
				  struct daos_cpd_disp_ent *dcde)
{
	...
	if (proc_op == CRT_PROC_FREE) {
		D_FREE(dcde->dcde_descs);
		return 0;
	}

	rc = crt_proc_uint32_t(proc, &dcde->dcde_rank);
	if (rc != 0)
		return -DER_HG;

	rc = crt_proc_uint16_t(proc, &dcde->dcde_tgt_idx);
	if (rc != 0)
		return -DER_HG;

	rc = crt_proc_uint16_t(proc, &dcde->dcde_desc_cnt);
	if (rc != 0)
		return -DER_HG;

	if (proc_op == CRT_PROC_DECODE) {
		D_ALLOC_ARRAY(dcde->dcde_descs, dcde->dcde_desc_cnt);
		if (dcde->dcde_descs == NULL)
			return -DER_NOMEM;
	}

	for (i = 0; i < dcde->dcde_desc_cnt; i++) {
		rc = crt_proc_struct_daos_cpd_disp_desc(proc,
							&dcde->dcde_descs[i]);
		if (rc != 0) {
			if (proc_op == CRT_PROC_DECODE)
				D_FREE(dcde->dcde_descs);
